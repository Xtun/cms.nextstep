...
class Banner extends Admin_Controller {
    ...
    public function __construct() {
        ...
        $this->_path_to_image = $this->banner_mapper->get_path_to_image();
    ...
    public function index() {
        ...
        $list           = $this->banner_mapper->get_categories($parent_id);
    ...
    public function add($parent_id = 0) {
        ...
        if ($this->form_validation->run()) {
            ...
            $this->banner_mapper->save($object, 'category');
    ...
    public function edit($id = 0, $parent_id = 0) {
        ...
        $object         = $this->banner_mapper->get_category($id, 'object');
        $data = array();
        $data['id']             = $object->id;
    ...
    public function delete($id, $parent_id) {
        $this->banner_mapper->delete((int)$id, 'category');
    ...
    public function show($parent_id) {
        $page_list    = $this->page_mapper->get_all_pages();
        $page_select    = $this->_get_pages_tree($page_list, $this->_templates['page_select_list'], $this->_templates['page_select_item'], (int)$parent_id);
        $list           = $this->banner_mapper->get_all_objects((int)$parent_id);
        $images         = array();
        if (sizeof($list) > 0) {
    ...
    public function add_image($parent_id = 0) {
        ...
        $page_list          = $this->page_mapper->get_all_pages();
        $page_select        = $this->_get_pages_tree($page_list, $this->_templates['page_select_list'], $this->_templates['page_select_item'], (int)$parent_id);
        $category           = $this->banner_mapper->get_category($parent_id);
        ...
        if ($this->form_validation->run()) {
            ...
            $this->banner_mapper->save($item, 'item');
    ...
    public function edit_image($id = 0, $parent_id = 0, $toggle_display = false) {
        ...
        if ((int)$id == 0) redirect(base_url().'admin/banner/'.$parent_id);
        $category               = $this->banner_mapper->get_category($parent_id, 'object');
        $item                   = $this->banner_mapper->get_object($id);
